/***************************************************************************************************************************** 
Source code file:      Prg05.c
Author, version, date: PSS ver 1.0 01.11.24
Program function:      Motor direction
...
LED2: ON - clutch motor is running CLOCKWISE, OFF - clutch motor is running ANTICLOCKWISE 
LED3: ON - brake motor  is running,           OFF - brake motor  is not running 
LED4: ON - brake motor  is running CLOCKWISE, OFF - brake motor  is running ANTICLOCKWISE
*/
...
uint8_t clutchMotorDirectionNow; 
...
uint8_t brakeMotorDirectionNow; 
   ...
   while (1)   
   {
      ...
      if (clutchMotorStateNow) 
		if (!IS1 & !IS2)							//  No current sensor error  
	       clutchMotorDirectionNow = getMotorDirection(); 
	  
	  
      brakeMotorStateNow  = getBrakeMotorState(); 
      if (brakeMotorStateNow) 
		if (!IS3 & !IS4)  
	       brakeMotorDirectionNow   = getMotorDirection(); 
      ...
      if (clutchMotorDirectionNow)
         LATFbits.LATF7 = 0;						//  LED2 RF7 ON, cluctch motor is turning clockwise 
      else
         LATFbits.LATF7 = 1;						//  LED2 RF7 OFF, clutch motor is running anticlockwise
      ...
      if (brakeMotorDirectionNow)
         LATAbits.LATA14 = 0;						//  LED4 RA14 ON, brake motor is turning clockwise
      else
         LATAbits.LATA14 = 1;						//  LED4 RA14 OFF, brake motor is turning anticlockwise 
   }  
}
void initializeMotors(void)
{   
   ANSB  = 0x0000;							//  Digital mode(D):0, Analog mode(A):1
   TRISA = 0x0000;							//  Output mode(O):0, Input mode(I):1, RA14:LED4(O) 
   TRISB = 0x3923;							//  RB0:PGEC(I), RB1:PGED(I), RB2:IS2(I), RB3:IN1(O), RB4:INH1(O), RB5:IS1(I), RB6:INH2(O), RB7:IN2(O), RB8:IS3(I), RB9:INH3(O), RB10:IN3(O), RB11:IS4(I), RB12:CurrentSensorInput(I), RB13:RadarSensorInput(I), RB14:INH4(O), RB15:IN4(O)
...
uint8_t getMotorDirection()
{
   if ((extInt1 & extInt2) | (extInt3 & extInt4))
   {
      ...
      if (bits3210 == 0b1101 || bits3210 == 0b0100 || bits3210 == 0b0010 || bits3210 == 0b1011) motorDirection = ANTICLOCKWISE;
      if (bits3210 == 0b1110 || bits3210 == 0b0111 || bits3210 == 0b0001 || bits3210 == 0b1000) motorDirection = CLOCKWISE;
      bits32 = bits10;
   }
